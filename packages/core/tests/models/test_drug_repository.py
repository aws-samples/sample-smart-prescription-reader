...
class MockDrugRepository(DrugInformationRepository):
    ...
    def search_drugs(self, terms: List[str], limit_kb: int = 100) -> List[DrugInformation]:
        ...
        for drug in self.drugs:
            for term in terms:
                if (term.lower() in drug.drug_name.lower() or 
                    term.lower() in drug.active_ingredient.lower()):
                    results.append(drug)
                    break
        return results
...
class TestSQLiteDrugRepository:
    ...
    def temp_db_path(self):
        ...
        cursor.executemany(
            'INSERT INTO drugs (drug_name, active_ingredient, strength, form) VALUES (?, ?, ?, ?)',
            test_data
        )
    ...
    def test_init_with_s3_path_download(self, mock_s3_client):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_s3_client.download_file.assert_called_once_with(
                'test-bucket', 'data/drugs.db', expected_path
            )
    ...
    def test_get_drug_name_list_from_s3(self, temp_word_list_path, mock_s3_client):
        ...
        mock_s3_client.get_object.assert_called_once_with(
            Bucket='test-bucket', Key='data/drug-list.txt'
        )
